...
<h4 id="item-revision">Item Revision</h4>
...
<h3 id="3-4客户化开发常用接口">3.4客户化开发常用接口</h3>
...
<h4 id="dba用户旁路功能打开">DBA用户旁路功能打开</h4>
...
<pre><code class="language-java">TCComponentType tcComptype =tcSession.getTypeComponent(&quot;N4MTMOPTrackingRevision&quot;);
</code></pre>
...
<pre><code>D:\Siemens\Teamcenter10\portal\portal.bat -nl zh_CN
...
<h4 id="菜单栏中添加一个新的菜单">菜单栏中添加一个新的菜单</h4>
...
<p><img src="/resources/杂七杂八/杂七杂八/TC/image-20200621220623531.png" alt="image-20200621220623531" /></p>

<h4 id="在右键上添加">在右键上添加</h4>
...
<h3 id="5-1-1无代码化定制">5.1.1无代码化定制</h3>
...
<p>使用解压缩包工具打开%TC _RO0T%\portal\plugins\com.teamcenter.rac.kernel_10000.1. 0.jar文件，替换\icons&rsquo;下的background. png文件。</p>
...
<p>(2)使用“数据集&hellip;..”查询，检索系统中现有的XMLRenderingStylesheet,找到Itemsummary数据集。</p>
...
<pre><code>release_ status_ list. I_ Released.ICON=images/C9_ IReleased.png
...
<p>HiddenPerspectives&ndash;隐藏功能模块</p>

<p><strong>AE_dataset_default_keep_limit</strong>数据集保存的版本数量。当超过这</p>

<p>个数值以后，老的版本即被删除。</p>

<h3 id="5-2-3访问权限管理">5.2.3访问权限管理</h3>
...
<pre><code>HSSFWorkbook wb= new HSSFWorkbook(); //创建WorkBook
...
<pre><code>FileInputStream inputStream= new FileInputStream (&quot; c: \\workbook. xls&quot;);
...
<pre><code>TCComponentBOMWindowType type = (TCComponentBOMWindowType)
...
<p>(2)设置版本规则
如果不使用默认的版本规则，需要替换其他版本规则的话，比如以下就是使用&rdquo;Latest Working”版本规则。</p>

<pre><code class="language-java">    TCComponentRevisionRuleType ruleType = (TCComponentRevisionRuleType)
...
<pre><code>topBomline = bomWindow.setWindowTopLine(topRevision.getItem(),topRevision,null, null);
topBomline.pack(); .
</code></pre>
...
<pre><code>TCComponentBOMLine bomline = (TCComponentBOMLine)
bomlineelement( );
...
<pre><code>private void BomDFS(TCComponentBOMLine parent) {
...
<pre><code>public static TCComponent[ ] getItem (String[ ]names, String [ ]values， String
...
<pre><code>public static String[ ] getProperties (TCComponentItem item, String [ ] properties) {
String [ ] values=null;
try {
...
<pre><code>public static TCComponentFolder getFolder (String foldername) {
TCSession
...
<pre><code>public void writeToExcel(String[ properties){
...
<pre><code>public static TCComponentDataset createDataset (TCSession session, String path,
